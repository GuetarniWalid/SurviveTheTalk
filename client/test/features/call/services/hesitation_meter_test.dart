...
void main() {
  ...
  test('confirmation offset compensates the 600 ms viseme silence window', () {
    // Story 7.6 — the offset MUST equal VisemeScheduler's silence-confirmation
    // window (600 ms): the meter arms one window AFTER the audio ended, so
    // subtracting it sets the gap start at the felt silence start → gap = the
    // true felt pause, by construction (NOT a free tuning knob).
    ...
    // gap = onset(4000) − gapStart(−600) = 4600 (the felt pause, not 4000).
