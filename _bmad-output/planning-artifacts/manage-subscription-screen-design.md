...
- **Restore sits ABOVE the primary CTA**, in the pinned block (`Restore → [msg] → 16 → [store-fail] → CTA → 16 → Terms·Privacy`). (Walid 2026-06-18 follow-up: tried below the CTA, settled on above as the cleaner reading order.) Quieter than the CTA; still its centered `StadiumBorder` text button, still reachable + enabled in every purchasable state (free/paid/loading) → Apple 3.1.1.
