...
### Review Findings (ultracode code-review, 2026-06-05)

Independent retrospective re-review of the merged + smoke-gated story — 6 lenses (Blind / Edge / Acceptance / Async-Correctness / Regression / Test-Quality), each finding skeptic-verified by 2 adversarial verifiers on the real source. **9 raised → 7 after merge: 1 → new story, 3 patched, 3 deferred, 0 dismissed.** Both 2026-06-04 fixes (the `met_count >= total` suppression + the dartdoc) re-verified intact.

**Decision → spun into [Story 6.25](6-25-fast-respeak-double-fail-hangup.md) (Walid 2026-06-05):**
- [x] [Review][Decision→6.25] **Await-not-cancel stacks TWO fail penalties on a fast re-speak → premature `character_hung_up` + Deviation #7 frame-forward race.** [checkpoint_manager.py:668-690 + 801-817] — CONFIRMED (2/2 verifiers) + empirically reproduced (await → patience=0/`is_hanging_up=True`; old cancel on identical FAIL/FAIL → patience=25/no hang-up). Turn 2's terminal precheck (`:607-609`) reads the meter BEFORE turn 1's in-flight fail lands → both turns take the non-terminal path; the await lands turn 1's fail and turn 2 then schedules its OWN fail → meter 0 → hang-up. The `:683` suppression only checks `met_count >= total`, so the hang-up case is missed and turn 2's frame is forwarded (`:690`) → the LLM reply races the silence-reason exit line. **The 2026-06-04 review's "patience-hangup unreachable" note (line 112 above + the inline comment `checkpoint_manager.py:680-682`) is factually incomplete** — it accounted only for the single awaited prior fail, not turn 2's own subsequent fail. The companion test gap (no real-`PatienceTracker` two-FAIL fast-respeak test) + an open product decision (once/twice drain) are folded into Story 6.25 (Task 3 + D1).

**Patch → APPLIED 2026-06-05 (doc/test-only, no production-behaviour change; gates re-run green: ruff clean, server pytest 638, flutter analyze clean + flutter test 406):**
- [x] [Review][Patch] **Inaccurate re-entrancy docstring** [checkpoint_manager.py `_serialize_then_classify`] — "process_frame is serialized per pipecat processor (no re-entrancy)" is false: pipecat 0.0.108 dispatches SystemFrames on a separate input task, so a BSF can re-enter `process_frame` while the await is parked. Benign today (re-entrant branches touch only `_bot_speaking`) but exactly the wrong-but-agreeing-assumption trap of server/CLAUDE.md §1 / Deviation #28. **Fixed:** docstring now states the accurate reason (TranscriptionFrame is non-system → serialized via the single process task; SystemFrames can re-enter but touch only `_bot_speaking`).
- [x] [Review][Patch] **AC5 reliable-duplicate count only asserted on a single flip** [test_checkpoint_manager.py] — multi-flip turns used the URGENT-only `_flip_envelopes` helper, blind to the queued reliable duplicates (mutation-proven: hoisting the reliable push out of the per-flip loop left the suite green). **Fixed:** added `test_multi_flip_emits_n_reliable_duplicates_matching_urgent` (a 2-flip turn → exactly 2 URGENT + 2 reliable, payloads value-equal).
- [x] [Review][Patch] **Client AC3 reconcile — SET-branch union/never-shrink only tested growing** [call_screen_test.dart] — the only SET test used a superset; a regression dropping the union would still pass. **Fixed:** added a union test (local {0,1,2,3} + server SET [0,1,4] → [0,1,2,3,4]: keeps 2,3 / adds 4).

**Deferred → [deferred-work.md](deferred-work.md) "code review of story-6.20 (2026-06-05)" (real but pre-existing / out-of-scope / zero current impact):**
- [x] [Review][Defer] Silent exception swallow in the `_serialize_then_classify` await (`gather(..., return_exceptions=True)`, no log) — the same swallow pattern pre-exists in `_run_classifier_blocking` / `cleanup` / `_schedule_classification`; not newly introduced.
- [x] [Review][Defer] AC4's enlarged shared `_SALIENT_STOPWORDS` also changed the pre-existing `lexical_overlap_pairs` heuristic — ZERO impact on all 6 real scenarios (max Jaccard 0.236); advisory build-time tool. Suggest a drift-only glue-word set + a pinning test.
- [x] [Review][Defer] AC5 "client dedupes the duplicate" asserted only in comments; the dedup machinery is pre-existing Story 6.10 code (untouched by 6.20). Low-value future widget guardrail.

_Story 6.20 stays `done` — the smoke gate (call_id=229) is unaffected: the one real defect is narrow and tracked as Story 6.25; the applied patches are doc + test only._
