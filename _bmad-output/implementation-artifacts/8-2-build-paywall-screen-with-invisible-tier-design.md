...
**On-device gate + D2 revert (2026-06-18).** Pixel 9 gate run on a free/3-call account (user 1 flipped on prod, backup `db.pre-8-2-d1-retest-20260618T080205Z.sqlite`, 74 `completed`→`failed` ids saved) + the release APK from `5e15885`: **✅ D1/FR29 PASSED** — the paywall auto-presented on the 3rd free call (the stale-snapshot fix verified LIVE). **❌ D2 swipe REGRESSED** — the custom route's hand-rolled drag killed the native swipe-to-dismiss; reverted (Walid-approved) to native `showModalBottomSheet`, swipe-block-during-purchase DEFERRED (deferred-work.md). Net after revert: **D1 + P1 + P2 + P3 shipped; D2 = native sheet (BACK gated via `PopScope`) + deferred**. Gates re-green: `flutter analyze` clean + `flutter test` **606**. The native-sheet swipe is identical to the already-shipped content-warning / difficulty sheets, so no further on-device gate is owed for the swipe; story stays `review` pending Walid's `review → done` call.
