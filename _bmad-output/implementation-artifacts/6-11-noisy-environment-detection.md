...
- [x] **Task 12 — Pre-commit + smoke gate** (AC: #9, #10)
  ...
  - [x] 12.4 — **WALID** — deploy VPS + 7-box smoke gate above ✅ 2026-05-30 (after 2 smoke-fixes: exit-line interruption race + "stop listening" InputGate)
  - [x] 12.5 — `review → done` after smoke gate proofs ✅ 2026-05-30 (Walid: "test OK, on peut valider")
...
- 2026-05-30 — **DONE.** Smoke gate validated on device by Walid ("test OK, on peut valider") after the two smoke-fixes below: with a loud continuous parasitic voice, detection fires → mic is muted (InputGate) → Tina delivers her full noisy-environment exit line uninterrupted → call ends `noisy_environment` → "Background voice was too loud" screen → gifted (not counted). `review → done`. Note (honest limitation, not a blocker): detection relies on Soniox diarization assigning distinct speaker ids on a single mic — robust when the parasite is a clearly separate voice, but can miss when the parasite *dominates* and becomes the cumulative-primary speaker (seen in earlier calls). The natural patience-based hang-up is the backstop there. The truly-robust fix (paid voice isolation) remains post-MVP per Story 6.9.
