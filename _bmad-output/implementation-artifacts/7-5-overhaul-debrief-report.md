...
- 🔁 **Post-review polish (2026-06-16, Walid feedback — applied directly per "fixes rapides"):** (1) **you-voice** — the debrief prompt now forbids "the user" / third-person and mandates second-person "you" on every learner-facing field (the `practice_prompt` stays first-person "I said…"); `DEBRIEF_PROMPT_VERSION` 2.1→2.2, prompt + authority doc in lockstep (drift guard green). (2) **checkpoint disclosure** — the breakdown collapses to a tappable "X of N reached" summary + a dark sheet (missed-first), so many-checkpoint scenarios no longer wall off the report (the old CALL-3 noise check is retired — device deferred). Gates: server pytest 905, client analyze + flutter 547. The Pixel 9 smoke gate (CALL 1 + CALL 2) now also covers these.
...
- `server/pipeline/prompts.py` — v2 `DEBRIEF_SYSTEM_PROMPT` + `DEBRIEF_PROMPT_VERSION = "2.2"`.
