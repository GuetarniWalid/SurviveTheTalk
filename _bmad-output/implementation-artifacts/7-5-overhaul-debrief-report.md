...
- 📋 **Remaining (next push, server-first per D6):** Task 3.1 (generator `_build_debrief_schema` v2 + `DEBRIEF_SYSTEM_PROMPT` v2 + amend the two authority docs `debrief-generation-prompt.md` §System Prompt + `debrief-content-strategy.md` v2 Q-amendment + bump `DEBRIEF_PROMPT_VERSION` "2.0" + update the `test_debrief_generator.py:125` verbatim drift guard in lockstep — NOTE Q3 reconciliation: card stays brief, explanation/examples live in the sheet) · Task 3.2 (thread checkpoint `{id,hint,met}` from bot teardown → assembly, NOT via LLM, do NOT break the BEGIN IMMEDIATE atomic claim) · Task 3.3 (`assemble_debrief` v2: version, checkpoints, id-based hesitation merge, derive `areas_to_work_on` titles from `areas`) · Task 3.4 (F2 `completed` end-reason / F3 per-item salvage / F4 `_MAX_TOKENS` resize) · Task 2.3-2.5 (HesitationObserver escalation-fix + id-tag, bot `on_data_received` device-gap collector, threshold) · Task 1 design-spec v2 doc · Task 4-5 client model+screen+sheets+copy · Task 6 gates+deploy+smoke.

### File List

- `server/models/schemas.py` — Story 7.5 v2 debrief wire contract: `DebriefOut` additive v2 fields + 3 new sub-models (`DebriefBetterPhrasing`, `DebriefCheckpoint`, `DebriefArea`) + `DebriefError`/`DebriefHesitation` additive fields (back-compat: v1 rows validate via defaults).
- `_bmad-output/implementation-artifacts/7-5-overhaul-debrief-report.md` — Status → in-progress; Task 2.0 checked; Dev Agent Record (spike findings, v2 contract blueprint, design direction, session notes).
- `_bmad-output/implementation-artifacts/sprint-status.yaml` — `7-5-overhaul-debrief-report`: ready-for-dev → in-progress.
