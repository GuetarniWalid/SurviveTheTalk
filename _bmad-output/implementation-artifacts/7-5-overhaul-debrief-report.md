...
- [ ] Task 2: Hesitation accuracy — D3-c client-authoritative measurement + server fallback (AC: 3)
  ...
  - [~] 2.3 Server: bot-side collector for device gaps beside `HesitationObserver`; teardown prefers device gaps (`source: "device"`), falls back to the v1 observer (`source: "server"`); escalation-blind-spot fix applied to the FALLBACK path too; `_merge_hesitations` → id-based pairing (schema echoes `hesitation_id`) — **SERVER-HALF DONE 2026-06-14** (escalation-blind-spot fix C2 + id-tagging C3 in `HesitationObserver`; `_merge_hesitations` id-based pairing in assembly; `source`/`resolved` carried). **Device-gap collector + teardown-prefers-device DEFERRED to ship WITH the client meter (2.1/2.2) as one co-designed/tested D3-c unit.**
  ...
  - [ ] 2.5 Tests: client meter unit tests (boundary sequences incl. re-speak/interrupt/mute), envelope contract test both sides, server collector + fallback-preference + id-pairing tests — **server observer escalation/id tests DONE (+5)**; client + envelope-contract tests pending with 2.1/2.2
