...
> Formal `/bmad-code-review` 2026-06-10 (3 adversarial layers on `opus`: Blind Hunter 20 raw / Edge Case Hunter 5 raw / Acceptance Auditor 4 raw = 29 raw → 26 unique after dedup). Every finding verified against the on-disk source (not just the diff) before triage; the zombie-reaping claim was additionally checked empirically on the live VPS (0 defunct processes after a day of pooled prod calls). Auditor: **zero AC violations** — positive confirmations for AC2/AC3/AC4/AC4b/AC5/AC6/AC7, D1–D4 and Tasks 1–6. Triage: 0 decision-needed / 7 patch / 4 defer / 15 dismissed. **All 7 patch findings APPLIED same day (6 actions, batch-apply)**; gates re-run green: ruff check + ruff format clean, full pytest **801 passed** (+2 review tests). None of the patches touches the smoke-gated behavior (drain/refs/teardown/startup-failure paths only) — the 2026-06-10 Pixel 9 gate stays valid.

- [x] [Review][Patch] Un-drained stdout PIPE on a served bot can wedge a live call + swallows its stdout logs [server/pipeline/bot_pool.py:226-258] — `_spawn_one` pipes stdout but `_await_ready` stops reading at the sentinel; for the rest of the bot's life (the whole call) nothing drains the pipe, so >64 KB of stdout output would block the bot's write and freeze the live call, and ANY stdout output is lost (the cold path inherits server stdio → journald). Fix: per-bot stdout drain task after readiness (re-log lines with a pid prefix, hold the proc reference until EOF, then `await proc.wait()`); also close stdin on the boot-timeout discard path (FD tidiness). [blind+edge]
- [x] [Review][Patch] Fire-and-forget tasks keep no strong reference [server/pipeline/bot_pool.py:164,210] — the event loop holds only weak refs to tasks: a GC'd refill task silently drops a refill until the 20 s maintainer tick; the stopped-mid-spawn `bot.terminate()` task can vanish the same way. Fix: a `_bg_tasks` set + done-callback discard (documented asyncio pattern). [blind+edge]
- [x] [Review][Patch] `stop()` terminates idle bots sequentially — worst case `size × 5 s` serial teardown [server/pipeline/bot_pool.py:288-289] — contradicts the module's own bounded-teardown claim (mirrors the janitor); `BOT_POOL_SIZE=8` of wedged bots = 40 s serial. Fix: `asyncio.gather` the terminates. [blind]
- [x] [Review][Patch] `BotPool.start()` unwrapped in the lifespan — an unexpected pool error could abort server startup [server/api/app.py:121] — `_spawn_one` maps every per-process failure to None, but `start()`/`_ensure_full` themselves aren't wrapped; AC4's intent ("the pool can never cost a call") extends to never costing the SERVER. Fix: try/except log-and-continue around `start()`. [auditor]
- [x] [Review][Patch] Test gap: the acquire mid-flight-death branch is uncovered [server/pipeline/bot_pool.py:153-162] — the trickiest race in the module (bot dies between liveness check and stdin write → BrokenPipe → pool miss → cold fallback) has a dedicated branch and zero tests. Fix: +1 test (monkeypatched `send_job` raising `BrokenPipeError`, assert `acquire` → False + warning log). [blind]
- [x] [Review][Patch] Test gap: the headline "concurrent fills never overshoot `size`" invariant has no concurrent test [server/pipeline/bot_pool.py:179-212] — the `_spawning` reserved-slot accounting is the riskiest concurrency code and is verified only by inspection. Fix: +1 test driving concurrent `_ensure_full`/refills against a counting spawn, asserting total spawns ≤ size. [blind]
- [x] [Review][Defer] Malformed job line after a pool hit = dead room with the 200 already returned [server/pipeline/bot.py:939] — deferred, by-design. `apply_parked_job` fails loud (uncaught ValueError → traceback in journald) and the job is built by our own `initiate_call` code, so the path needs server-side drift to trigger; the janitor sweeps the orphaned `pending` row. Hardening idea if it ever fires: pool-side post-acquire early-death watch. [edge]
- [x] [Review][Defer] Pool test stubs share one `STUB_JOB_FILE` [server/tests/test_bot_pool.py] — deferred, latent test-hygiene: today only one job is ever delivered per test; a future test delivering jobs to TWO bots of the same pool would race two stubs onto one file. Note for test authors. [blind]
- [x] [Review][Defer] Steady-state opening-latency observability is `LATENCY_PROBE`-gated [server/pipeline/bot.py:625-641] — deferred, pre-existing (Story 6.8 design): the always-on signals are the `[pooled]`/`[cold]` + `acquired parked bot pid=` lines; the per-call first-audio delta needs the probe env flipped. Candidate for the post-MVP observability bundle. [auditor]
- [x] [Review][Defer] `_spawning` decrement skippable if a refill task is cancelled during the finally's lock acquire [server/pipeline/bot_pool.py:202-204] — deferred, unreachable-in-practice: refill tasks are only cancelled at loop shutdown (pool already dead); the maintainer cancel in `stop()` releases the lock before the finally needs it. Revisit only if refill tasks ever get cancelled mid-run. [blind]

**Dismissed (15, verified false positives / by-design):** lock-held `drain()` can't block (pipe empty + one job line ≪ 64 KB high-water; a timeout-miss would ADD a double-join risk) [blind+edge]; refill-task burst churn is bounded no-ops via reserved-slot accounting [blind]; `except (CancelledError, Exception)` on the awaited just-cancelled maintainer = the janitor P9 idiom [blind]; stdout banner interleaving handled (substring sentinel + readline loop + boot timeout) [blind]; job env value shapes guarded by producer code + key whitelist [blind]; hand-rolled `--url/--room/--token` check is STRICTER than `required=True` (rejects empty strings) [blind]; redundant double ready-assert in tests (stubs don't die spontaneously) [blind]; per-module `Settings()` is the codebase idiom [blind]; post-`stop()` acquire degrades to cold spawn, children die with the systemd cgroup (pre-existing restart semantics) [blind]; "zero CPU while parked" holds on both platforms (blocked read), prod is Linux [blind]; `-m pipeline.bot` cwd assumption = parity with the pre-existing cold path, live-validated [blind]; parked-vs-cold env-snapshot divergence needs post-boot env mutation that doesn't exist [blind]; served-bot zombie claim empirically falsified (0 defunct on the VPS; asyncio child watcher reaps) [edge]; constructor-signature deviation self-disclosed + sanctioned [auditor]; "Spawned … [pooled]" verb keeps the deliberate common grep prefix [auditor].
...
- 2026-06-10 — **Formal `/bmad-code-review` COMPLETE (post-smoke; the review had been skipped in the dev flow).** 3 adversarial layers on `opus` + source-level + live-VPS verification of every finding: zero AC violations, 0 decision-needed / 7 patch / 4 defer / 15 dismissed (see `### Review Findings`). All 7 patches applied: per-bot stdout drain task (kills the un-drained-PIPE live-call wedge + restores lost stdout logs + pins the proc handle), strong refs + shutdown reaping for fire-and-forget pool tasks, concurrent `stop()` teardown, lifespan `start()` try/except (a pool failure can never abort the server), +2 regression tests (mid-flight `send_job` death → cold-fallback miss; concurrent fills never overshoot `size`). Gates green: ruff check + format clean, full pytest **801 passed**. Defers logged in `deferred-work.md` §"code review of story-6.26 (2026-06-10)". Patches don't touch the smoke-gated behavior — the Pixel 9 gate stays valid; story stays `done`.
