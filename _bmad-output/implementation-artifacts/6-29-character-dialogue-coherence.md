...
### Review Findings (formal /bmad-code-review 2026-06-11 — 3 layers: Blind Hunter 18, Edge Case Hunter 7, Acceptance Auditor 8 raw → 22 unified)

- [x] [Review][Decision] **AC9 latency proof half-missing — `LATENCY_PROBE=1` on-device before/after never run.** The per-turn `checkpoint_verdict_wait` INFO log shipped and was measured live (208-440 ms typical, one 802 ms budget miss correctly failed open, calls 276/277) and Walid signed the feel check (script item 7), but the AC's first clause — a `LATENCY_PROBE=1` on-device before/after — appears nowhere in the record. _RESOLVED (Walid 2026-06-11): accept the wait-log + signed feel check as the AC9 measurement — recorded as Deviation D8._
- [x] [Review][Patch] Case/format-variant mood tags are SPOKEN — `<mood:Anger>`/`<MOOD:SMIRK>` match neither `_MOOD_TAG_RE` nor `_could_be_tag_prefix`, released as literal text to TTS + transcript (AC8 letter breach); fix = case-insensitive match + lowercase the value [server/pipeline/reply_sanitizer.py:105-120] _APPLIED (+3 tests)_
- [x] [Review][Patch] AC9 observable lies on the crash path — `verdict_landed` stays `True` in the generic `except Exception` branch, so a crashed classify task logs as a landed verdict [server/pipeline/checkpoint_manager.py:1092-1100] _APPLIED (crash test now pins `verdict_landed=False`)_
- [x] [Review][Patch] Unguarded end-of-reply flush — a straggling `LLMFullResponseEndFrame` after an interruption reset triggers `_flush_end_of_reply()` with `_in_llm_response` already False → spurious `reply_sanitizer_empty_reply_dropped` INFO per barge-in [server/pipeline/reply_sanitizer.py:267-271] _APPLIED (+1 test)_
- [x] [Review][Patch] Record formal Deviations D5-D7 — (D5) golden sweep partial-pass vs AC4's "sweep PASSES" letter (currently only in Completion Note #7); (D6) harness keeps the RAW line when a simulated reply sanitizes to empty (`clean_line or character_line`, calibration_engine.py:643) while prod drops the turn — golden==prod divergence on the exact P2 shape; (D7) barge-in during the bounded hold drops the held TF (pipecat latest-line-wins, Completion Note #5) — adjacent to AC7's "no dropped turn" [story file] _APPLIED (Deviations D5-D8)_
- [x] [Review][Patch] File List missing the `49de9bb` files — `scenarios.py` (waiter drink-terms STT bias) + `test_scenarios.py` are in the review diff and Change Log but absent from File List [story file] _APPLIED_
- [x] [Review][Patch] Stale EmotionEmitter doc references — `bot.py:582` "BEFORE emotion_emitter", the now-dangling 6.4 "env vars above" comment (bot.py:114), `exchange_classifier.py:21` "EmotionEmitter stays on Qwen via OpenRouter" (doubly stale), docstring mentions in environment_monitor/endpoint_watchdog/dtln_audio_filter/llm_provider, `config.py` "optional field" wording [server/pipeline/bot.py:582 + 6 files] _APPLIED (comment-only)_
- [x] [Review][Defer] `SCENARIO_CHARACTER` is now a dead producer — `routes_calls` still sets it and `_PARKED_JOB_ENV_KEYS` (bot.py:925) still forwards it, but nothing reads it server-side since the retirement (a test pins the non-read); removing producer + parked-key + re-pinning the test = cleanup beyond this diff — deferred, out-of-diff cleanup

Dismissed as noise/intended (14, highlights): unpaired-`*`/unclosed-`(` truncating the reply tail = the documented `finish()` trade-off (same acceptance family as AC2's whole-drop, observable via INFO, releasing suppressed text would reintroduce P2); `*emphasis*` vs action ambiguity = inherent lexical-backstop limit, charter rule 7 is the primary defense; silent-reply-still-emits-mood = pinned intended by test (a completed generation's mood ≠ a barge-in half-reply); envelope-before-audio timing = client verified order-indifferent + smoke-validated; TTSSpeakFrame docstring claim = VERIFIED correct on pipecat 0.0.108 (DataFrame, not TextFrame); serialize-before-wait total-hold = pre-existing 6.20/6.25 semantics; whitespace-only chunk forward = deliberate (dropping it would word-join TTS chunks); validator-vs-clamp, double-space residue, >24-char invalid split tag, wall-clock drive-test sleeps, STT-bias pin-test style, unicode bracket variants.
...
- [x] **Server logs clean on the happy path.** `journalctl -u pipecat.service -n 100 --since "10 min ago"` during the script below: no ERROR/Traceback; (D1) per-turn wait log present; (D2) no `emotion_emitter` lines remain; sanitizer strip events logged when triggered.
  - _Proof:_ Calls 276 (2026-06-11 07:41) + 277 (08:04): zero ERROR/Traceback; `checkpoint_verdict_wait waited_ms=208-440 budget_ms=800 verdict_landed=True` on every judged turn (one 802 ms budget miss correctly failed open); `reply_sanitizer_mood_emit` per reply; no `emotion_emitter` lines. Only WARNINGs were two pre-existing, by-design-handled provider events: one classifier ReadTimeout (→ patience-neutral fail-open) and one Cartesia silent TTS stall on call 277's exit line (`cartesia_tts_watchdog_fired` + `hang-up TTS timeout` backstops fired as designed — follow-up chip filed for an exit-line retry).
- [x] **Pixel 9 on-device script (below) validated by Walid.** _Signed by Walid 2026-06-11 ("Ok, c'est ok pour moi") on calls 276+277: drink checkpoint ticked on "Okay, so cola." (post STT-bias fix `49de9bb`), all 6 checkpoints + completion + survived end, ticks landing before/with the reply (D1), face following her lines (D2), no spoken meta, no answered-question re-asks, no verbatim re-recitation after credit. Known exception, accepted: call 277's goodbye line was generated (330 ms, contextual) but Cartesia returned no audio — pre-existing 6.13 stall class, NOT a 6.29 behavior; retry hardening = separate chip._
...
- **D5 (formalized by the 2026-06-11 review) — AC4's "golden-only sweep PASSES" is met for the gating fixture, not the letter of the fleet.** waiter (the hand-authored `reviewed: true` GATING fixture) + 3 others PASS; cop_hard_01 PASSES on `--force` re-run (seed-boundary flake); mugger_medium_01 fails STABLY on a PRE-EXISTING permissive opening criteria (first-ever fleet golden run — zero 6.29 judge inputs changed, AC5). Full forensics in Completion Note #7; deferred-work entry + scenario-authoring chip filed.
- **D6 (formalized by the 2026-06-11 review) — harness keeps the RAW line when a simulated reply sanitizes to empty.** `calibration_engine.py:643` (`clean_line or character_line`) diverges from prod's whole-drop on the exact P2 shape — deliberate (the simulated learner needs a non-blank line to react to; prod has a silence concept, the text harness does not). Confined to the rare simulated pure-meta reply; documented inline.
- **D7 (formalized by the 2026-06-11 review) — a barge-in DURING the bounded hold drops the held TF.** Pipecat cancels a processor's in-process frame on user-turn-start interruption (latest-line-wins — same class as the pre-existing terminal/stacked blocking paths); the shielded classify still lands its verdict. AC7's "no dropped turn" is pinned by the drive test for the late-TF path; the 6.20/6.25 stacked/coalescing path stays the safety net for the re-speak window. Discovered + recorded in Completion Note #5.
- **D8 (review decision, Walid 2026-06-11) — AC9's `LATENCY_PROBE=1` on-device before/after was not run; accepted substitute evidence:** the per-turn `checkpoint_verdict_wait` live measurements (208-440 ms typical, one 802 ms budget miss failing open, calls 276/277) + the signed smoke-gate feel check (script item 7, PRD 2 s ceiling held).
...
- `server/pipeline/scenarios.py` — waiter drink-terms STT bias (same-gate fix `49de9bb`, call 276 "cola"→"caller" miss)
- `server/tests/test_scenarios.py` — drink-terms bias pin (same-gate fix `49de9bb`)
- Review patches (2026-06-11): `reply_sanitizer.py` (case-insensitive mood tag + guarded end-flush, +4 tests), `checkpoint_manager.py` (`verdict_landed=False` on crashed task, test extended), stale-comment cleanup in `bot.py` / `exchange_classifier.py` / `environment_monitor.py` / `endpoint_watchdog.py` / `dtln_audio_filter.py` / `llm_provider.py` / `config.py`
...
- 2026-06-11 — **Formal /bmad-code-review COMPLETE → review → done** (the review was the last owed gate; smoke signed same morning on calls 276+277). 3 adversarial layers (Blind Hunter / Edge Case Hunter / Acceptance Auditor), 33 raw findings → 22 unified: 1 decision (AC9 probe → accepted as D8), 6 patches APPLIED (case-insensitive mood-tag match, `verdict_landed=False` on crashed task, guarded end-of-reply flush, Deviations D5-D8 formalized, File List completed, stale EmotionEmitter comments cleaned in 7 files), 1 defer (`SCENARIO_CHARACTER` dead producer → deferred-work), 14 dismissed. Auditor verdict: What-NOT-to-do 0 violations, D1-D4 match the code exactly, AC5 crediting byte-untouched. Post-patch gates: server pytest **875** (+4 new tests), ruff clean, flutter analyze clean + **451 passed**, zero client diffs.
...
- 2026-06-11 — **Pixel 9 smoke gate SIGNED by Walid** (calls 276+277; see gate boxes). Two same-day adjacent fixes shipped during the gate: `49de9bb` (waiter STT drink-terms bias — call 276's "cola"→"caller" miss) committed separately; exit-line TTS-stall retry deferred to a spawn-task chip (pre-existing Cartesia 6.13 class). Story stays `review` — **waiting ONLY on the formal code review** (Walid runs it with a separate agent); the reviewer flips `review → done` per the root-CLAUDE.md discipline.
