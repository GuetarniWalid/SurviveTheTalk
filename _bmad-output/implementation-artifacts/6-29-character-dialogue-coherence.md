...
> ⚠️ **DECISION PASS PENDING (Walid).** Three decisions below (D1/D2/D3) gate the implementation scope. `dev-story` MUST NOT start until they are resolved and recorded in this file. D1 deliberately surfaces TWO documented, opposing positions — per the sprint-status instruction, the story does NOT pre-decide it.
...
**Resolution:** _PENDING — Walid picks (a)/(b)/(c)._
...
**Resolution:** _PENDING — Walid picks: include the face half in THIS story, or defer it to a follow-up story (then this story ships only D1/D3 + charter + sanitizer)._
...
**Resolution:** _PENDING — Walid picks (a)/(b)/(c). Note: full persona recalibration on Llama is explicitly OUT of scope either way (separate deliberate follow-up)._
...
- [ ] **T3 (D1) — Wait-for-verdict in CheckpointManager (AC7, AC9)**
  - [ ] Extend `process_frame`'s non-terminal branch ([checkpoint_manager.py:803-863](../../server/pipeline/checkpoint_manager.py)): after `_serialize_then_classify`, await THIS turn's in-flight task (reuse the `_run_classifier_blocking` shape) before falling through to `push_frame`. If D1=(c), wrap in the wait-budget `asyncio.wait_for` with stale-steering fallthrough.
...
- [ ] **T5 (D3) — Character model re-pin (AC10)**
  - [ ] (b): VPS `.env` `CHARACTER_MODEL=llama-3.3-70b-versatile` + service restart; note the 100k TPD watch item.
  - [ ] (c): add `CHARACTER_BASE_URL`/`CHARACTER_API_KEY` settings (default empty → fall back to `resolve_llm_*` globals) consumed ONLY by `build_main_llm`; needs the provider key from Walid (ASK — never invent credentials).
