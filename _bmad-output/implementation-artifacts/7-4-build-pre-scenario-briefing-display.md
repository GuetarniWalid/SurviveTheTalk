...
### Review Findings (code review 2026-06-12 — 3 layers: Blind Hunter / Edge Case Hunter / Acceptance Auditor, diff `dfdf1a2..5220826`, 21 raw → 20 unique findings)

- [x] [Review][Patch] `hasBriefingContent` counted content under non-canonical keys — a corrupt-but-dict row (e.g. `{"foo": "bar"}`) passes the server's `dict | None` net and would gate into a blank dossier; the getter now checks exactly the 3 keys the screen renders (AC-C1's "one of the three values" now literal) + model test [client/lib/features/scenarios/models/scenario.dart:90]
- [x] [Review][Patch] Stale-capture race in `_onScrollMetrics` — two opposite metrics notifications in one frame could latch the wrong hairline state (only the first enqueues a post-frame callback, which then applied its closure-captured value); the desired state now lives in a `_hairlineDesired` field re-read inside the callback [client/lib/features/briefing/views/briefing_screen.dart:131]
- [x] [Review][Patch] Dead `_scrollController` removed — created/attached/disposed but never read (the hairline is notification-driven); flagged independently by 2 layers; the mechanism swap vs the spec's "via the scroll controller" wording is recorded as declared deviation #6 [client/lib/features/briefing/views/briefing_screen.dart]
- [x] [Review][Patch] Misleading `hasBriefingContent` doc comment — claimed "an absent or all-empty briefing never pushes the BriefingScreen", wrong for the browse card-tap entry which pushes regardless of content (AC-C7); the comment now states the gate-only contract [client/lib/features/scenarios/models/scenario.dart:84]
- [x] [Review][Patch] No render test for the `briefing == null` legacy payload — only the all-empty map was covered; added the null variant (browse entry renders lockup + footer, zero eyebrows) [client/test/features/briefing/views/briefing_screen_test.dart]
- [x] [Review][Patch] AC-C7's double-PUSH half untested — the hub suite covered double-pop but never a double-tap on the call icon / card; added both (after one dismiss the hub must be back — a stacked second route would still show the stub) [client/test/features/scenarios/views/scenario_list_screen_test.dart]
- [x] [Review][Patch] Hairline re-evaluation on metrics changes untested (spec probe: "not just on first build") — added a resize toggle test: fits → shrink (appears) → restore (hides) [client/test/features/briefing/views/briefing_screen_test.dart]
- [x] [Review][Patch] §Layout-spec viewport contract (360×800 above-the-fold, BINDING) was unverified — and unverifiable under the Ahem test font (square glyphs wrap ~2× wider than Inter, the probe failed spuriously); the suite now loads the real Inter family in `setUpAll` and PROVES the authored waiter briefing fits (no hairline ⇔ nothing scrolls) [client/test/features/briefing/views/briefing_screen_test.dart]
- [x] [Review][Defer] Malformed 2xx `/calls/initiate` envelope (null / wrong-typed `data`) would raise a TypeError that escapes the `on ApiException`-only catch in `_startCall` — the tap silently does nothing [client/lib/features/scenarios/views/scenario_list_screen.dart] — deferred, pre-existing (Story 6.1 catch structure untouched by 7.4; the server envelope is Pydantic-built, so it takes a server bug to trigger; recorded in deferred-work.md)

Dismissed as noise / by-design: 11 — hairline predicate is the spec's literal `maxScrollExtent > 0`; Dart wildcard params + `flagsCollection` fine on the `^3.11.0` SDK floor; seeder strictness "boot risk" disproven (prod deploy green, 6/6 keys-ok — fail-fast IS AC-S3's intent); route redirect untested = declared deviation #1 (no router harness exists, auditor-verified); system-back-during-exit pop race = pre-existing app-wide class; `go(root)` fallbacks = spec-mandated by AC-C2; decorative `:scenarioId` path param; AC-C8 grep-lint brittleness (`.trim` omission = declared deviation #3); word-budget arithmetic lives in the BINDING spec comment block (spec errata, not code); `AppColors.hairline` global token = AC-C8's sanctioned ONE token; back arrow scrolling away with the body = the spec's own layout (system back remains).
...
- **Declared deviations:**
  ...
  6. **Hairline visibility is driven by `ScrollMetricsNotification`, not "the scroll controller"** (review-acknowledged 2026-06-12) — same `maxScrollExtent > 0` predicate, but the notification fires on first layout AND on metrics changes (rotation, text-scale) with zero controller plumbing; the vestigial `ScrollController` that shipped in the dev commit was removed by the review patches.
