...
- [x] [Review][Defer] 🟠 AC1 — refreshed `prod_snapshot.sqlite` is owed at the Task 8 deploy (cannot be done pre-deploy); `test_migrations.py` loosened to a repo-count assertion meanwhile. Must land before `review → done`. [`tests/fixtures/prod_snapshot.sqlite`] — deferred (deploy-gated)
