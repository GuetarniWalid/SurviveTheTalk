...
**AC-C10 — Entry + exit transitions per the design timing.** *(AMENDED 2026-06-10 post-smoke-gate — Pixel-9-validated; see Completion Notes §Smoke-gate fix.)*
Entry (as shipped): the in-call canvas cuts to the shared `#1E1F23` scene break the moment `CallEnded` lands — the originally-specified "call screen fade-out 500 ms `easeIn`" kept the stale dial surface visible under the overlay and read as a second hang-up screen on device — then the overlay content fades in 500 ms `easeOut` (1000 ms route total; the 0–500 ms backdrop fade is retained but visually inert, dark-over-dark). Exit: ~900 ms crossfade (600/600, 300 ms overlap) to the debrief. Pushed as `pushReplacement` (forward-only, UX-DR10).
...
### Deploy
- [x] **Task 11 — Deploy + Smoke Gate.** Deploy (applies migration 012 → refresh+commit snapshot), then Walid's Pixel 9 gate below. *(Deployed 2026-06-10: CI success on `8e60ea0`, snapshot refreshed+committed `99be9cf`. Pixel 9 smoke gate VALIDATED by Walid 2026-06-10 — failure variant + success variant + latency masking + airplane-mode notice path, all on the post-fix `a2abadd` build. ALL 9 gate boxes checked below.)*

### Review Findings (code review 2026-06-10)

> Formal 3-layer adversarial review (Blind Hunter / Edge Case Hunter / Acceptance Auditor) of `18b1f05..2f6e447`, post-smoke-gate. 21 raw findings → 4 patches, 3 deferred, 11 dismissed (mostly blind-layer false positives: the timing seams exist, `EndCallResult.durationSec` pre-exists, corrupt-JSON→500 is AC-S4's explicit mandate + the 5.1 convention, inline identity styles match the sibling screens, the avatar/track color swap is the LOCKED Dev-Notes decision, detail-route `end_phrases=None` is the documented deliberate choice).

- [x] [Review][Patch] `CallEndedScreen` fixed Column lacks the Story 5.4 overflow wrapper its sibling dial surface uses — RenderFlex overflow at large text scales / small viewports [client/lib/features/call/views/call_ended_screen.dart:407-508] — **APPLIED**: `LayoutBuilder + SingleChildScrollView + ConstrainedBox(minHeight) + IntrinsicHeight` wrap (byte-pattern of `_buildDialSurface`), + overflow test at 320×480 / textScaler 1.5
- [x] [Review][Patch] Empty-role degraded mode (unknown `riveCharacter`, catalog miss): SR announcement reads "Name, . Call Ended" + an empty 19px role line renders [client/lib/features/call/views/call_ended_screen.dart:349-360] — **APPLIED**: announcement builder extracted to `@visibleForTesting callEndedAnnouncement` (bare name when role empty) + `if (role.isNotEmpty)` on the role line, + unit test
- [x] [Review][Patch] Seeder accepts any `end_phrases` shape — a malformed block (string/list/missing variant/empty string) seeds silently, then `GET /scenarios` 500s `SCENARIO_CORRUPT` for every user; the daily-scenario VPS flow bypasses the CI test that would catch it [server/db/seed_scenarios.py:72] — **APPLIED**: fail-fast shape validation in `_row_from_yaml` (mapping + 3 canonical non-empty variants, extras allowed, absent stays legal/NULL) + `test_seed_scenarios_rejects_malformed_end_phrases`
- [x] [Review][Patch] AC-C10 + the Entry/exit Completion Note still describe the pre-smoke-gate-fix "call screen fade-out" beat — internally contradicted by §Smoke-gate fix + the shipped `SizedBox.shrink()` scene break (doc amendment, not code) [this file] — **APPLIED**: AC-C10 amended to the shipped scene-break entry; Completion Note parenthetical corrected
- [x] [Review][Defer] Hold timers don't pause on app backgrounding (overlay/cap can fire unseen) + `accessibleNavigation` sampled once at entry (mid-hold toggle keeps the 3 s min) [client/lib/features/call/views/call_ended_screen.dart:228-246] — deferred, rare lifecycle polish; smoke-validated UX
- [x] [Review][Defer] Element-level screen-reader semantics partial vs the design a11y table (raw "02:47" on traversal, unlabeled avatar, default progress-bar semantics, container label traversed alongside children) [client/lib/features/call/views/call_ended_screen.dart:397-406] — deferred, the combined live-region announcement satisfies AC-C13/P-5; element-level pass is post-MVP a11y polish
- [x] [Review][Defer] Envelope `survival_pct` (patience-ratio formula) is now fully unread by the client — retire or unify server-side when Story 7.3 lands [server/pipeline/patience_tracker.py:1327-1357] — deferred, pre-existing wire field
...
- [x] **On-device: failure variant.** End a call (tap hang-up, or character hangs up ≥30 s). Overlay shows name/role/avatar, `MM:SS`, a **red** % + bar matching the in-call checkpoint progress, and the scenario's red **server** phrase; holds ≥3 s; fades to debrief. _Proof:_ Walid Pixel 9 sign-off 2026-06-10 (build with the `a2abadd` Calling-screen fix) — Waiter, voluntary hang-up at 00:27: red 33% + partial bar + "You walked out" (screenshot in session); single hang-up screen post-fix.
- [x] **On-device: success variant.** Complete a scenario → **green** 100% + the grudging "survived" phrase; auto-fades to debrief. _Proof:_ Walid Pixel 9 sign-off 2026-06-10 — "le succès… tout est ok".
- [x] **Latency masking.** Hold feels deliberate (no overlay spinner); debrief appears already-rendered after the crossfade. _Proof:_ Walid Pixel 9 sign-off 2026-06-10 — no spinner reported, hold + crossfade validated across both variants.
- [x] **Notice path intact.** `network_lost`/`noisy_environment`/very-short gifted call still shows `CallEndedNoticeScreen` (no regression). _Proof:_ Walid Pixel 9 sign-off 2026-06-10 — airplane-mode mid-call test OK ("même avec le mode avion").
...
- **Entry/exit transitions (AC-C10)** — `CallEndedScreen.route()` is a 1000 ms `PageRouteBuilder`: backdrop fade-in over 0–500 ms `easeIn` (since the §Smoke-gate fix the route underneath already renders the bare `#1E1F23` scene break, so this fade is visually inert — the earlier "call screen fading into #1E1F23" reading predates the fix), content fade-in 500–1000 ms `easeOut`; on exit the same route fades out over the first 600 ms of the debrief route's 900 ms window via `secondaryAnimation` while the debrief fades in over 300–900 ms — the design's 300 ms crossfade overlap.
...
| 2026-06-10 | **Pixel 9 smoke gate VALIDATED by Walid** (post-fix `a2abadd` build): failure variant ✅, success variant ✅, latency masking ✅, airplane-mode notice path ✅ — all 9 gate boxes checked, Task 11 complete. Status STAYS `review` (6.23 precedent: the formal `/bmad-code-review` has not run yet). **`review → done` now awaits ONLY the code review** — no device re-test needed once its findings are resolved. |
| 2026-06-10 | **/bmad-code-review COMPLETE — `review` → `done`.** 3-layer adversarial review (Blind Hunter / Edge Case Hunter / Acceptance Auditor) of `18b1f05..2f6e447`: 21 raw findings → **4 patches applied** (Story 5.4 overflow wrapper + 320×480/1.5× test; degraded-mode announcement via `callEndedAnnouncement` + empty-role guard + unit test; seeder `end_phrases` fail-fast shape validation + `test_seed_scenarios_rejects_malformed_end_phrases`; AC-C10/Completion-Note doc amendment), **3 deferred** to deferred-work.md (timer lifecycle/a11y-flip polish, element-level SR semantics, envelope `survival_pct` retirement), **11 dismissed** (blind-layer false positives / spec'd decisions / family conventions — auditor verified all 17 ACs + 6 Decisions + 7 claimed deviations match the code). Gates: ruff clean + **pytest 806** (+1); analyze clean + **flutter test 451** (+2). Smoke gate already signed off → flip per the standing root-CLAUDE.md rule. |
