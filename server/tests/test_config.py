...
def test_settings_classifier_model_defaults_to_openai() -> None:
    """Story 10.6 (2026-06-29) — `classifier_model` defaults to `gpt-4.1-mini`
    (migrated to OpenAI, which has NATIVE strict json_schema for the §4 judge
    law). The default is what prod uses when `CLASSIFIER_MODEL` is unset.

    `clear=True` so a developer's exported `CLASSIFIER_MODEL` can't shadow the
    default and make the test silently pass.
    """
...
def test_settings_debrief_model_defaults_to_openai() -> None:
    """Story 10.6 (2026-06-29) — `debrief_model` defaults to `gpt-4.1-mini`
    (migrated to OpenAI; native strict json_schema incl. the ["string","null"]
    debrief union — server/CLAUDE.md §4)."""
    env = {**REQUIRED_ENV_VARS, "JWT_SECRET": "0" * 32}
    with patch.dict(os.environ, env, clear=True):
        s = Settings(_env_file=None)  # type: ignore[call-arg]
        assert s.debrief_model == "gpt-4.1-mini"
...
def test_settings_character_model_defaults_to_openai() -> None:
    """Story 10.6 (2026-06-29) — the character LLM defaults to OpenAI
    `gpt-4.1-mini` (env-overridable via CHARACTER_MODEL). `emotion_model` is
    LEGACY since Story 6.29 (mood co-generated by the reply LLM, EmotionEmitter
    retired); nothing reads it at runtime, so its default is left untouched and
    only checked to confirm it still parses. `clear=True` for a hermetic env."""
    env = {**REQUIRED_ENV_VARS, "JWT_SECRET": "0" * 32}
    with patch.dict(os.environ, env, clear=True):
        s = Settings(_env_file=None)  # type: ignore[call-arg]
...
def test_settings_llm_provider_defaults_to_openai() -> None:
    """Story 10.6 (2026-06-29) — the single LLM provider switch defaults to
    OpenAI; llm_api_key empty by default (the resolver falls back to
    groq_api_key, and prod sets LLM_API_KEY to the OpenAI key)."""
    env = {**REQUIRED_ENV_VARS, "JWT_SECRET": "0" * 32}
    with patch.dict(os.environ, env, clear=True):
        s = Settings(_env_file=None)  # type: ignore[call-arg]
