...
def compose_spike_character_led_instruction(
    *,
    ...
    holistic = (
        ...
        "expect them to keep talking. So on a clear insult or contemptuous "
        "dismissal, give at MOST ONE short warning ('talk to me properly or we're "
        "done here'); if they disrespect you even once more after that, END the "
        "call. If the insult is blatant or aggressive, end it IMMEDIATELY, with no "
        "warning. Never hand out more than one 'last chance'. \n"
        ...
        "Ending is done WITH a final spoken line, so it never conflicts with any "
        "rule about always speaking or never going silent. When you decide to end "
        "it, deliver that final in-character line and then write the marker "
        "<end_call> right after it (place it just before your mood tag). The marker "
        "is removed before the other person hears you — never say it aloud, never "
        "explain it. Do NOT end over genuinely clumsy, frustrated, or awkward "
        "language that is NOT aimed at you as a person — a learner hunting for "
        "words, a mumbled 'I don't know', or mild frustration with the situation is "
        "fine and must never end the call — and NEVER end while the conversation is "
        "still making real progress. While you are continuing, never write that "
        "marker."
    )
    composed = base + "\n\n" + coherence_charter + "\n\n" + holistic
    if mood_tag_directive:
